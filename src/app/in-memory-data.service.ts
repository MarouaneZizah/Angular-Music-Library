import { InMemoryDbService } from 'angular-in-memory-web-api';

export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    const artists = [
      { id: 1, name: "Hans Zimmer", genre : "Soundtracks", Image:"../assets/img/hansZimmer.jpg" },
      { id: 2, name: "Eminem", genre : "Rap", Image:"../assets/img/eminem.jpg" },
      { id: 3, name: "Michael Jackson", genre : "POP", Image:"../assets/img/michaelJackson.jpg" },
      { id: 4, name: "Mozart", genre : "Classical", Image:"../assets/img/mozart.jpg" },
      { id: 5, name: "AC/DC", genre : "Rock", Image:"../assets/img/acdc.jpg" },
      { id: 6, name: "Bob Marley", genre : "Reggae", Image:"../assets/img/bobMarley.jpg" },
    ];
    return {artists};
  }
}
